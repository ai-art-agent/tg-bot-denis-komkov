...
1. В браузере откройте: **https://ваш-поддомен.ваш-домен.com/miniapp**  
   Должна открыться страница выбора формата (карточки: групповые занятия, диагностическая консультация, Pro, личная работа). Если страница не открывается — проверьте, что на ВМ запущен `robokassa-server` (`sudo systemctl status robokassa-server`) и что туннель активен (`sudo systemctl status cloudflared`).
