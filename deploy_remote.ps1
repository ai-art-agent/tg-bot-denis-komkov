...
# Только стек Дениса: бот + robokassa-server-denis + cloudflared-quick (robokassa-server Энхель не перезапускаем).
$remoteCmd = "cd ~/tg-ai-denis-komkov && git pull && venv/bin/pip install -r requirements.txt && sudo cp deploy/tg-ai-denis-komkov.service /etc/systemd/system/ 2>/dev/null; sudo cp deploy/robokassa-server-denis.service /etc/systemd/system/ 2>/dev/null; sudo cp deploy/cloudflared-quick.service /etc/systemd/system/ 2>/dev/null; sudo cp deploy/robokassa-server.service /etc/systemd/system/ 2>/dev/null; sudo systemctl daemon-reload && sudo systemctl restart tg-ai-denis-komkov && sudo systemctl restart robokassa-server-denis && sudo systemctl restart cloudflared-quick && echo '' && echo '--- Bot Denis ---' && sudo systemctl status tg-ai-denis-komkov --no-pager && echo '' && echo '--- Robokassa Denis ---' && sudo systemctl status robokassa-server-denis --no-pager && echo '' && echo '--- Cloudflared quick ---' && sudo systemctl status cloudflared-quick --no-pager && echo '' && echo '--- URL для кабинета Robokassa (из PUBLIC_BASE_URL в .env) ---' && venv/bin/python print_robokassa_urls.py 2>/dev/null || echo '(задайте PUBLIC_BASE_URL в .env)' && echo '' && echo '(robokassa-server Enhel not restarted; use: systemctl status robokassa-server)'"
